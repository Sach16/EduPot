...
public class CustomExpandableListAdapterForLessonsShare extends BaseExpandableListAdapter {
    ...
    private static RecyclerShareListener m_cClickListener;
    private static boolean m_cIsFlatView;

    public CustomExpandableListAdapterForLessonsShare(Context context, List<String> listDataHeader,
                                                      HashMap<String, Group> listChildData,
                                                      HashMap<String, Role> listDataChildFlat,
                                                      HashMap<Integer, Object> pSelectionItems,
                                                      boolean pIsFlatView, RecyclerShareListener pListener) {
        ...
        if (m_cIsFlatView) {
            if (((Role)m_cSelectionItems.get(groupPosition)).isSelected()) {
                ...
                if (((Role) m_cSelectionItems.get(groupPosition)).getShares().equals(1))
                    lblImageViewTick.setImageDrawable(m_cContext.getResources().getDrawable(R.drawable.tick_shared));
                else
...
//            lblLLMain.setBackgroundColor(m_cContext.getResources().getColor(R.color.white));
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        try {
            ...
            CustomExpandableListAdapterForLessonsShare.InnerListAdapter listAdapter = new CustomExpandableListAdapterForLessonsShare.InnerListAdapter(m_cContext,
                    groupPosition,
                    lGroup != null ? lGroup.getRoles() : null,
                    lRole != null ? lRole.getUsersShares() : null,
                    m_cIsFlatView);
    ...
    public class InnerListAdapter extends RecyclerView.Adapter<CustomExpandableListAdapterForLessonsShare.InnerListAdapter.DataObjectHolder> {

        private Context m_cObjContext;
        private List<Role> mObjRoleList;
        private List<UsersShare> mObjUsersList;
        ...
        public class DataObjectHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnLongClickListener {
            ...
            public void onClick(final View view) {
                switch (view.getId()) {
                    case R.id.CHILD_LL:
                        if (mInnerIsFlatView) {
                            if(((Role)(m_cSelectionItems.get(groupPosition))).getUsersShares().get(getPosition()).isSelected()) {
                                ((Role) (m_cSelectionItems.get(groupPosition))).getUsersShares().get(getPosition()).setSelected(false);
                                if (((Role) (m_cSelectionItems.get(groupPosition))).getUsersShares().get(getPosition()).getShares().equals(1))
                                ...
                                ((Role)(m_cSelectionItems.get(groupPosition))).getUsersShares().get(getPosition()).setSelected(true);
        ...
        @Override
        public CustomExpandableListAdapterForLessonsShare.InnerListAdapter.DataObjectHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            View lView;
            if (mInnerIsFlatView) {
            ...
            CustomExpandableListAdapterForLessonsShare.InnerListAdapter.DataObjectHolder ldataObjectHolder = new CustomExpandableListAdapterForLessonsShare.InnerListAdapter.DataObjectHolder(lView);
            return ldataObjectHolder;
        }

        @Override
        public void onBindViewHolder(CustomExpandableListAdapterForLessonsShare.InnerListAdapter.DataObjectHolder holder, int position) {
            if (mInnerIsFlatView) {
                try {
                ...
                if (((Role) (m_cSelectionItems.get(groupPosition))).getUsersShares().get(position).isSelected()) {
                    holder.tickIv.setImageDrawable(m_cContext.getResources().getDrawable(R.drawable.tick_blue));
                } else {
                    if (((Role) (m_cSelectionItems.get(groupPosition))).getUsersShares().get(position).getShares().equals(1))
                ...
                try {
                    holder.leftLabel.setText(mObjUsersList.get(position).getUsers().getFirstName() + " " +
                            mObjUsersList.get(position).getUsers().getLastName());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
